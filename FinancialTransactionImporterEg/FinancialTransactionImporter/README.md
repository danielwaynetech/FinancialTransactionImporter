...
**Stack:** .NET 9 API · SQLite · React + TypeScript · Docker Compose
...
The value is a [.NET custom date format string](https://learn.microsoft.com/en-us/dotnet/standard/base-types/custom-date-and-time-format-strings).
...
- **Separation of Concerns:** The solution follows Clean Architecture principles with three layers — Core (domain), Infrastructure (data access), and API (presentation). The Core has zero infrastructure dependencies.
- **Strict CSV Validation:** A single invalid row causes the entire upload to be rejected — atomicity is enforced at the service layer before any database writes occur.
- **Configurable Parser:** Delimiter and date format are injected via `IOptions<CsvSettings>` making the parser fully testable in isolation without touching configuration files.
- **Duplicate Prevention:** TransactionId uniqueness is enforced both within an uploaded file (in-memory HashSet) and at the database level (unique index).
- **Pagination:** Server-side pagination is implemented to handle large datasets without loading all records into memory.
